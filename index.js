...
bot.on("guildDelete", guild => {
  const leaveembed = new discord.MessageEmbed()
    ...
    .setFooter(`New Guild Size: ${bot.guilds.cache.size} Servers |`)
    .setTimestamp()
    .setThumbnail(guild.iconURL());

  let botjoinleavemessagechannel = "788623018753785856";

  const sendchannel = bot.channels.cache.get(botjoinleavemessagechannel);
...
bot.on("guildCreate", guild => {
  ...
  let botjoinchannel = "788622948739186699";

  const sendchannel = bot.channels.cache.get(botjoinchannel);
  if (!sendchannel) return;
  ...
  try {
    if (
      ...
      return message.channel.send(
        `**My Prefix In This Server is - \`${prefix}\`. MY Developer = \`ROWDY KING\` **`
      );
    }
  } catch {
    return;
...
bot.on("guildMemberAdd", async member => {
  ...
  let welcome = await jimp.read(
    "https://t.wallpaperweb.org/wallpaper/nature/1920x1080/greenroad1920x1080wallpaper3774.jpg"
  );
